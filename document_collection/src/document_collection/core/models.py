...
class DocumentSource(BaseModel):
    ...
    @classmethod
    def determine_source_type(cls, v: Any, info: Any) -> SourceType:
...
class DocumentMetadata(BaseModel):
    ...
    @classmethod
    def determine_format(cls, v: Any, info: Any) -> DocumentFormat:
...
class CollectionRequest(BaseModel):
    ...
    convert_to_markdown: bool = Field(
        default=True, description="Whether to convert document to markdown"
    )
    preserve_original: bool = Field(
        default=False, description="Whether to keep original file alongside markdown"
    )
    overwrite_existing: bool = Field(
        default=False, description="Whether to overwrite existing files"
    )
    metadata: dict[str, Any] | None = Field(
        default=None, description="Additional metadata to attach"
    )
...
class CollectionResult(BaseModel):
    ...
    output_path: Path | None = Field(None, description="Path to converted document")
    original_path: Path | None = Field(None, description="Path to original document")
...
class BatchCollectionRequest(BaseModel):
    ...
    convert_to_markdown: bool = Field(
        default=True, description="Whether to convert documents to markdown"
    )
    preserve_original: bool = Field(
        default=False, description="Whether to keep original files alongside markdown"
    )
    overwrite_existing: bool = Field(
        default=False, description="Whether to overwrite existing files"
    )
    parallel_processing: bool = Field(
        default=True, description="Whether to process documents in parallel"
    )
...
class BatchCollectionResult(BaseModel):
    ...
    results: list[CollectionResult] = Field(
        default_factory=list, description="Individual collection results"
    )
