...
class Configuration:
        ...
        for env_var, config_key in env_mapping.items():
            ...
            if env_value is not None:
                ...
                if config_key in ["max_workers", "retry_attempts"]:
                    ...
                    self._config[config_key] = env_value.lower() in [
                        "true",
                        "1",
                        "yes",
                        "on",
    ...
    def max_workers(self) -> int:
        ...
        return int(self.get("max_workers", DEFAULT_MAX_WORKERS))

    @property
    def timeout(self) -> float:
        """Get timeout in seconds."""
        return float(self.get("timeout", DEFAULT_TIMEOUT))

    @property
    def retry_attempts(self) -> int:
        """Get number of retry attempts."""
        return int(self.get("retry_attempts", DEFAULT_RETRY_ATTEMPTS))

    @property
    def retry_delay(self) -> float:
        """Get retry delay in seconds."""
        return float(self.get("retry_delay", DEFAULT_RETRY_DELAY))
